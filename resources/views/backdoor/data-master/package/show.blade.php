...
<x-layouts.backdoor.index
  ...
  <x-slot:content>
    <div class="w-full space-y-6" x-data="ShowPackage" x-init="setPackageInfo({{ json_encode(['slug' => $package->slug, 'category_id' => $package->category_id, 'category_name' => $package->category?->name, 'name' => $package->name, 'is_active' => $package->is_active, 'features' => $package->features->pluck('description')->values()->all()]) }})">
      ...
      <div class="bg-stone-50 border border-stone-200 p-6 md:p-8">
        ...
        <div class="flex justify-between items-start gap-4 pb-6">
          ...
          <button
            ...
            class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-stone-700 bg-white border border-stone-300 hover:bg-stone-50 transition active:scale-[0.97] cursor-pointer">
        ...
        <div x-data="{ open: false }">
          <span class="text-[10px] md:text-xs font-semibold tracking-wider text-stone-500 uppercase">Keterangan
            Global</span>
          <ul class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3">
        ...
        <x-backdoor.table.header>
          ...
          <x-slot:right>
            <x-backdoor.table.add-button
              x-on:click="openVariantDrawer('{{ $package->slug }}')"
              text="Tambah Varian" />
          </x-slot:right>
        </x-backdoor.table.header>
        {{-- table header end --}}
            ...
            <tr
              ...
              <x-backdoor.table.cell>
                <div class="flex flex-col gap-1" x-data="{ open: false }">
                  <div class="flex items-center gap-2">
                    ...
                    <x-shared.whatsapp-badge alpine="item.is_whatsapp_only" />
                  </div>
              ...
              <x-backdoor.table.actions>
                <x-backdoor.table.action-item
                  ...
                  x-on:click="closeDropdown(); editVariant(item, window.__packageSlug)"
                  text="Edit" />
                <x-backdoor.table.action-item
                  x-bind:disabled="state.isLoading"
                  color="text-red-600"
      ...
      <x-backdoor.data-master.package.package-drawer-form
